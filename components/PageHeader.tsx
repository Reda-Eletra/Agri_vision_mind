...
export const PageHeader: React.FC<PageHeaderProps> = ({ title, subtitle, imageUrl, eyebrow, eyebrowIcon, children }) => {
  return (
    ...
    >
      ...
      <div className="relative z-10 flex min-h-[18rem] flex-col justify-end p-5 md:p-8 lg:p-10">
        <div className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_auto] lg:items-end">
          <div className="max-w-4xl rounded-[1.5rem] border border-white/12 bg-white/10 p-5 backdrop-blur-xl md:p-6">
            {eyebrow ? (
            ...
            <h1 className="text-3xl font-extrabold tracking-[-0.04em] text-white md:text-5xl">{title}</h1>
            <p className="mt-4 max-w-2xl text-sm leading-7 text-white/78 md:text-lg">{subtitle}</p>
          </div>

          {children ? <div className="w-full lg:w-auto lg:self-end">{children}</div> : null}
        </div>
      </div>
    </section>
  );
